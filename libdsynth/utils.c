...
static void (*write_note2)(int16_t *, void **, note_t *, uint32_t);
    ...
    for (nr_t *n = bnr->nr; n != NULL; n = n->next) {
      ...
      case SYM_NOTE:
        ...
        for (note_t *note = n->note; note != NULL; note = note->next) {
          // fprintf(stderr, "n ");
          write_note2(pcm, args, note, pcm_seek);
        }
        // putchar('\n');
        pcm_seek += n->length;
        continue;
...
void write_block(int16_t *ipcm, void **args, bnr_t *bnr,
  ...
  pcm = ipcm;
  write_note2 = write_note;
  write_block2(args, bnr);
  pcm_seek = 0;
  scope = 0;
  pcm = NULL;
}
